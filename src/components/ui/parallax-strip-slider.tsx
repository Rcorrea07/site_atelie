...
// Adaptado para o Ateliê Sy Corrêa: a fonte de display vem do next/font
// (--font-instrument) em vez de um <link> injetado em runtime.
"use client";
...
export type ParallaxStripSliderProps = {
  ...
  showCounter?: boolean;
  /** Enable the click-to-navigate overlay and its circular cursor. */
...
export default function ParallaxStripSlider({
  ...
  // Toque OU janela estreita: numa janela de desktop estreita o título em
  // `w-1/3` ficava apertado demais e o cursor circular atrapalhava.
  useEffect(() => {
  ...
  useEffect(() => {
    if (!showControls || isCompact) return;
    ...
    return Array.from({ length: stripCount }, (_, i) => (
      ...
      >
        ...
        >
          ...
          >
            {/* eslint-disable-next-line @next/next/no-img-element -- a mesma foto e clonada em N tiras e animada por clip-path; next/image nao ajuda aqui */}
            <img
              src={slide.src}
              alt=""
              draggable={false}
  ...
  return (
    ...
    >
      ...
      >
        ...
        >
          {activeSlide.chapter ??
            `Coleção ${String(caption + 1).padStart(2, "0")}`}
        </span>
      </div>
      ...
      >
        <h2
          ref={titleRef}
          className={`pointer-events-none flex items-end overflow-hidden text-6xl leading-none tracking-[-0.02em] sm:text-7xl lg:text-8xl ${
        ...
        {showCounter && (
          <span
            ref={counterRef}
            className={`label pointer-events-none flex shrink-0 items-center ${
              stacked
      ...
      {showControls && total > 1 && !isCompact && (
        <div
          ref={cursorRef}
